...
module private ArgParserRuntime_MapArgs =
    open System
...
/// Methods to parse arguments for the type MapArgs
[<RequireQualifiedAccess ; CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
module MapArgs =
    let parse' (getEnvironmentVariable : string -> string option) (args : string list) : MapArgs =
        let helpText () =
            [
                (sprintf
                    "%s  %s%s%s"
                                ...
                                (fun entry ->
                                    ...
                                    if parser_sep < 0 then
                                        ...
                                        ((parser_key
                                          ...
                                          (entry.Substring (parser_sep + 1)) |> (fun x -> System.Int32.Parse x)),
                                         parser_key)
                                )
                            )
    ...
    let parse (args : string list) : MapArgs =
...
[<RequireQualifiedAccess ; CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
module MapInUnion =
    let parse' (getEnvironmentVariable : string -> string option) (args : string list) : MapInUnion =
        let helpText () =
            [
                ...
                Leaves =
                    [
                        ...
                        }
                        {
                            ...
                            Arity = ArgParserRuntime_MapArgs.ErasedArity.One
                            Repeatable = false
                            Requirement = ArgParserRuntime_MapArgs.ErasedRequirement.Required
                            TypeDescription = ""
                            Help = None
                        }
                    ]
                Tree =
                    (ArgParserRuntime_MapArgs.ErasedTree.Sum (
                        (0,
                         [
                             ("Deploy",
                              ArgParserRuntime_MapArgs.ErasedTree.Product (
    ...
    let parse (args : string list) : MapInUnion =
        parse' (System.Environment.GetEnvironmentVariable >> Option.ofObj) args
