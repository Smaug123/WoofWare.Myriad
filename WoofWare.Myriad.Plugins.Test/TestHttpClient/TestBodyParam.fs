...
module TestBodyParam =
    ...
    let ``Body param of string`` () =
        task {
            ...
            let! observedUri = api.CreateUserString ("username?not!url%encoded")
    ...
    let ``Body param of stream`` () =
        task {
            ...
            let! observedContent = api.CreateUserStream (stream)
    ...
    let ``Body param of HttpContent`` () =
        task {
            ...
            let! response = api.CreateUserHttpContent (content)
            response |> shouldEqual "oh hi"
    ...
    let ``Body param of byte arr`` (case : string) =
        task {
            ...
            let! observedContent =
                match case with
                | "ByteArr" -> api.CreateUserByteArr (contents)
                | "ByteArr'" -> api.CreateUserByteArr' (contents)
                | "ByteArr''" -> api.CreateUserByteArr'' (contents)
    ...
    let ``Body param of serialised thing`` () =
        task {
            ...
            let! result = api.CreateUserSerialisedBody (expected)
    ...
    let ``Body param of primitive: int`` () =
        task {
            ...
            let! result = api.CreateUserSerialisedIntBody (3)
    ...
    let ``Body param of primitive: Uri`` () =
        task {
            ...
            let! result = api.CreateUserSerialisedUrlBody (Uri "https://mything.com/blah")
